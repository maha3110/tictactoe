...
def alpha_beta_pruning(state, depth, alpha, beta, player):
    ...
    :param alpha: best option for MAX
    :param beta: best option for MIN
    ...
    for cell in empty_cells(state):
        x, y = cell[0], cell[1]
        state[x][y] = player
        score = alpha_beta_pruning(state, depth - 1, alpha, beta, -player)
        state[x][y] = 0
        score[0], score[1] = x, y
        ...
        if player == COMP:
            ...
            if best[2] > alpha:
                alpha = best[2]  # alpha acts like max in MiniMax
            if alpha >= beta:
                break  # beta cut-off
        else:
            if score[2] < best[2]:
            ...
            if best[2] < beta:
                beta = best[2]  # beta acts like min in MiniMax
            if alpha >= beta:
                break  # alpha cut-off
...
class Ui_TicTacToe(object):
    ...
    def retranslateUi(self, TicTacToe):
        ...
        TicTacToe.setWindowTitle("TicTacToe - AlphaBeta Algo")
    ...
    def ai_turn(self):
        """
        It calls the alpha_beta_pruning function if the depth < 9,
